...
`PATH`、`LANG`、`LC_ALL`、`SOURCE_DATE_EPOCH` 是 schema 固定的 deterministic runner baseline，不是 Component 可申请的 environment role，也不得出现在 `[[environment]]`：`PATH` 只由已解析 executable allowlist 生成，后三者固定为 `C.UTF-8`、`C.UTF-8`、`0`。根据 [ADR 0011](docs/adr/0011-bind-cargo-driver-environment.md)，`CargoDriverEnvironmentV1` 固定 planner 的 channel override、`CARGO_CACHE_RUSTC_INFO=0`、`CARGO_HOME=/rust-agent/cargo-home`、`CARGO_INCREMENTAL=0`、`CARGO_NET_OFFLINE=true`、`CARGO_TARGET_DIR=/rust-agent/target`、`RUSTC=/rust-agent/toolchain/bin/rustc`，选中 Host linker 时的 `COMPILER_PATH=/rust-agent/tools`，以及 build 阶段的 exact Target-only `CARGO_ENCODED_RUSTFLAGS` 与 `TMPDIR=/rust-agent/tmp`；[ADR 0012](docs/adr/0012-bind-target-linker-execution.md) 将包含 Target-linker 执行语义的 backend semantic version 推进为 6。Pinned Cargo 可以把该 exact closed driver environment、自身 unit metadata variables 与 selected environment roles传给 build script/proc macro；它们不得来自 ambient input或 Component role，任一缺失、额外或 value drift在 Cargo前失败。Policy 可以提供 composition 未使用的资源，但 runner 只挂载/暴露 union 实际引用的最小子集以及固定 toolchain/source/target/temp/deterministic-environment baseline；缺少命中、类型错配、同 id 不同 variable/value、Component 未声明而 dependency-family gate 检出实际使用，都使 production build 在执行任何 build script 前失败。该 requirement→policy resolution 的 logical id/content/version/role projection、逐项 provenance 和 aggregate identity digest进入 build manifest；含 concrete mapping/full policy item digest 的记录只进入 enforcement-attestation payload，二者都不进入 composition runtime effect set。
...
Schema 4 production policy 保留一个 optional closed `host-linker` bundle：一个 linker executable id 和按 id 排序、无重复的 helper executable id 集合；每个 id 都必须分别命中带 digest/version 的 `[[executable]]`。Normalized build requirements 对该 bundle 只能全不选或全选，partial selection 必须在 Cargo 前失败。选中时，schema 5 planner 与 build 固定传入 ordered Cargo command-line config `target-applies-to-host=false`、`host.<build-triple>.linker="/rust-agent/tools/<linker-id>"`、`host.<build-triple>.rustflags=["-Clinker-features=-lld"]`，并固定启用 `-Z target-applies-to-host`、`-Z host-config` 和 `COMPILER_PATH=/rust-agent/tools`；这些 Host-only settings 关闭 rustc 对 schema-selected helper 的隐式 self-contained LLD 替换且不得泄漏到 composition Target units。Component 不得覆盖。Schema 4 还以 sorted target-exact `[[target-linker]]` 把 `wasm32-unknown-unknown` 映射到独立 id/path/digest/version，并要求该 target 在任何 Cargo side effect 前恰有一个 fixed LLD probe-compatible identity；duplicate target/id、与 generic executable/read-input/environment id overlap、invalid identity或缺失均 fail closed。Schema 5 planner/build只通过 `target.wasm32-unknown-unknown.linker="/rust-agent/target-tools/<id>"` 暴露该文件，它不进入 `PATH` 或 `COMPILER_PATH`。Host/Target linker 的分离选择及 logical paths 进入 schema 3 `BuildEnforcementIdentity`；backend semantic version为 6。Planner digest、exact Cargo invocation 与受监督 rustc observation绑定完整配置，后者还要求 Target-linker实际执行次数恰等于产生链接输出的 Target rustc unit数；concrete Host paths 仍仅进入 full policy/attestation。Linker、helper、startup object、linker script 与 compiler runtime 必须由 policy/runtime closure 分别绑定，完整 Rust sysroot保持non-executable，禁止 wrapper、alias、runtime-tree executable fallback 或 ambient PATH/filesystem discovery。

普通 executable version probe 仍只能执行被探测文件且以 `/` 为只读 working directory。Schema 4 `ProductionInputIdentityRequest` 以 closed `host-linker`/`host-linker-helper`/`target-linker` file role 区分 probes，旧 schema 不得获得该语义；schema 3 observation绑定exact ordered role/id/digest/argv/result。Target-linker使用固定 `-flavor wasm --version` 单 executable只读 probe，并独立descriptor-mount到 `/rust-agent/target-tools/<id>`。Host-linker helper（例如会在 `--version` 内部创建临时文件并派生 `ld` 的 GCC `collect2`）使用独立 schema-owned probe class：每个 probe 获得新的 runner-owned `/rust-agent/probe-tmp` writable mount、固定 `COMPILER_PATH=/rust-agent/tools`，且 descendant allowlist 只能是 fully selected Host-linker bundle；network 仍隔离，probe 后丢弃该目录。该 probe 的 observation 必须绑定主 executable、实际 descendant digest、logical working directory 与 writable mount；不得把这项例外授予 bundle 外 executable、复用 build target/temp，或暴露 Host temp/ambient helper。
...
生产 runner 清空 ambient `RUSTFLAGS/RUSTDOCFLAGS/CARGO_ENCODED_RUSTFLAGS/RUSTC_WRAPPER`、Cargo profile override、代理变量与未由 schema baseline、`CargoDriverEnvironmentV1` 或已解析 `[[environment]]` role 产生的 build environment；所有 runner-owned variable 使用固定 logical value且不得读取同名 ambient value。Build-owned `CARGO_ENCODED_RUSTFLAGS` 必须且只能是 Target-scoped exact `--sysroot=/rust-agent/toolchain`。选中 schema-owned Host linker bundle 时，baseline 还包含固定 `COMPILER_PATH=/rust-agent/tools`，且 Cargo Host config把 exact linker和唯一 `-Clinker-features=-lld`只交给无 `--target` 的 Host build-script/proc-macro compilation；有 `--target` 的 composition Target compilation必须恰有一个 explicit target sysroot且没有该 Host flag。未选 bundle时 Host config与 Host flag都必须不存在。Build script/proc macro 可以读取 composition、verified source、toolchain和显式 read-input，可以在 target/temp 写入；可以观察 Cargo 1.97.1继承的 exact closed driver variables与 Cargo自身 unit metadata，但不能读取 ambient home、proxy、credential、wrapper、profile override、unselected role或其它 secret。Target linker、rustc flags、canonical Cargo resolution config、target-fact/custom-spec digest、toolchain/input/executable/environment-role 的 path-free identity、sandbox semantic class/version 和规范化 logical environment 通过 `BuildEnforcementIdentity`、exact Cargo invocation identity 与 execution observation进入 build-output digest；concrete mappings、完整 normalized policy 与 enforcement evidence 只进入 attestation。依赖预置环境可以跳过联网 fetch，但仍执行 locked source verification。
...
Phase 2 的完成声明必须逐项满足以下 acceptance identity；这些 identity 只是把本节及 Sections 4–7、23、28–31、42、46–47、50、52–53 已有约束变成可核对映射，不改变阶段边界：

```text
[P2-AC-01] core/runtime-api expose the complete effect-free Phase 2 ownership contracts, explicit target-compatible Clock/Sleeper/Spawner primitives and owner-scoped draining without ambient executor dependence
[P2-AC-02] lightweight session API independently owns the complete lower-level persistence/query/journal/read-store DTO closure and compiles before agent in every feature mode without a Session provider or agent dependency
[P2-AC-03] runtime-api opaque composition/binding assembly is the only way to bind exact generated providers/consumers, projected authority and paired request-journal call authority; callers cannot forge contexts, stamps, receipts or prepared calls
[P2-AC-04] single- and multi-provider model routing validates default versus explicit-per-request mode before journal/provider effects and streams only an exact proof-bound PreparedModelCall with cancellation, deadline and byte budgets
[P2-AC-05] generated App/Agent factory ownership atomically publishes and removes complete live scopes, retains them until owner teardown and rolls back every construction/publication failure
[P2-AC-06] one active turn plus bounded queued admission implements same-id same-fingerprint retry joining, completed-result replay, conflict rejection, exact targeted cancellation, deadline wakeup and async idempotent shutdown quiescence
[P2-AC-07] public Agent event feeds capture subscriber registration and baseline/high-water in one publisher linearization point, carry typed agent/lifecycle/request identity and enforce aggregate count/event/byte bounds with one terminal Lagged or Closed outcome
[P2-AC-08] lifecycle observers use transactional before-publish veto and pre-reserved bounded post-commit delivery with timeout, cancellation, error/panic containment and bounded owner-scoped shutdown
[P2-AC-09] commands expose the empty guarded Phase 2 dispatcher and check lifecycle/admission before returning UnsupportedOperation
[P2-AC-10] model-replay, model-host, driver-direct and the selected runtime adapter obey their Component ownership, coexistence, effect and primitive declarations on every claimed Phase 2 target
[P2-AC-11] generated Phase 2 composition emits exact selected bindings, observer contributors, runtime primitive projections and closed single/multiple-provider runtime routing schema while excluding every unselected Session/provider/heavy dependency
[P2-AC-12] minimal-pure deterministically composes, locks and builds in isolation and its generated artifact proves publication rollback, proof-before-every-stream and the complete Request -> LanguageModel -> Response lifecycle
```
