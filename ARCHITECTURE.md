...
- discovery fixture 的 `[patch]`、`[replace]`、named registry、workspace/ancestor Cargo config在第一次 metadata前拒绝，ambient `CARGO_HOME` source replacement不可见；受支持 graph 的 discovery/lock/build必须逐字节复用 generated `cargo-resolution.json`/`.cargo/config.toml`，任一 source/config digest漂移在 Cargo前失败；production fetch fixture 证明 `cargo fetch --locked` 可调用 exact pinned `rustc -vV`，wrapper/其它 rustc argv/codegen/source binary 均被 runner拒绝；
...
- emitted library 被独立 Host Cargo graph通过唯一 alias编译；pre必须用 pinned Cargo planner分别产生 emitted standalone与 final Host的 `HostCargoUnitGraph`，缺少受信 unit-graph interface或退回单一 `cargo metadata --filter-platform`时 production失败；
