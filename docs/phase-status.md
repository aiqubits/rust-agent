...
| 4 — local execution providers | Complete | Slices 4.1–4.10 implement the effect-free bounded filesystem API, exact projected resource bootstrap, production Linux descriptor-relative `fs-read-local`/`fs-local`, provider-neutral `tool-fs`, the closed process/confinement/shell/terminal API, the Linux sandbox policy planner, verifier-only Linux subprocess provider, pure `shell-local`/`terminal-local` adapters, API-only bounded `tool-shell`, and the generated local composition root. The resolver owns the closed issuer/verifier infrastructure routes and requires a bounded fail-closed `[confinement]` ceiling; the security manifest records that ceiling. Generated Agent preparation creates the authority pair before namespace I/O, performs the exact projected one-shot filesystem preparation before Agent identity allocation, constructs the selected DAG in resolved order, initializes typed Component owners in order before publication, and rolls back or shuts them down in reverse order. An isolated locked generated composition is deterministic, records exact graph presence/absence, expands target-scoped workspace dependency inheritance into a standalone snapshot, creates an Agent, executes the replay Tool driver path, and drains Agent/App lifecycle. Local filesystem, process, shell, terminal and tool security regressions remain mapped; the real subprocess/PTY gate is assigned to the privileged Ubuntu runner because this local host disables the required unprivileged namespace setup. Every Phase 4 acceptance requirement is mapped to a named local or CI gate. |
