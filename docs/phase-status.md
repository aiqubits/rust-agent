...
| 3 — tool execution plane | In progress | Slices 3.1–3.12 implement the bounded Tool DTO/output/policy and opaque registration boundary, PermissionPolicy/Approval, permission-default, provider-free Prompt/Compaction/Attachment/Spill/Telemetry contracts, a bounded provider-neutral model Tool-call stream DTO, the API-only `driver-tools` loop, and the guarded reference-monitor core: sealed provider snapshots, private deterministic registry, exact ToolCall issuer/verifier/proof, pure model-origin planning, proof-before-policy/approval/provider dispatch, effect-ceiling enforcement, a metadata-only tool-executor-guarded wrapper, bounded typed execution middleware, invariant-lifetime Nested/Command-origin sessions, model-origin rolling bounded scheduling, explicit nested cost accounting, and exact generated AgentContext/consumer-edge proof wiring. Model Tool-call events retain checked names/arguments/provider ids, count toward the aggregate output budget and cannot occur after completion; the model API remains independent of Tool implementations. `driver-tools` fixes a Tool snapshot before each model request, derives normalized ids from composition/Agent/request/step/ordinal identity, rejects malformed JSON or duplicate provider ids before Tool effects, and admits only the fixed `plan → journal → seal → bounded batch execute` path while preserving model order and bounding steps, history bytes, concurrency and usage. Generated scope assembly records at most one selected-driver → ToolExecutor edge, issues the paired authority only after exact edge validation, keeps the Tool issuer in AgentContext and gives the selected scope factory only an opaque provider-bound verifier envelope; direct Tool authority issuance is private. Tool call cost is a checked build-reviewed registration field included in sealed identity; Nested and Command origins consume their shared count/cost authority atomically before permission or provider callbacks. The scheduler admits only one exact cancellation/deadline/runtime lineage, treats unknown/default as global exclusive, permits explicitly parallel calls up to a checked hard cap, serializes equal derived resource keys, stops new dispatch after cancellation/deadline failure, records a guarded terminal outcome for every admitted call and returns outcomes in model order. Nested calls reuse the pinned registry and exact guarded raw dispatch while inheriting root identity, cancellation, deadline, output/effect ceilings and bounded depth/count/cost/cycle state. Command grants are opaque, exact-permit borrowed and carry bounded call/cost/output/effect authority; `prepare_command` additionally binds Agent/lifecycle, executor digest and the exact clock/sleep projection without exposing runtime primitives. Compile-fail tests prevent authority forgery, cloning, lifetime escape and conversion to model sessions. Later phases will connect non-empty CommandDispatcher issuance to its journal gate. Phase 3 still requires generated tool composition and graph-presence/absence evidence. |
