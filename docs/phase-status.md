...
| 3 — tool execution plane | In progress | Slices 3.1–3.6 implement the bounded Tool DTO/output/policy and opaque registration boundary, PermissionPolicy/Approval, permission-default, provider-free Prompt/Compaction/Attachment/Spill/Telemetry contracts, and the guarded reference-monitor core: sealed provider snapshots, private deterministic registry, exact ToolCall issuer/verifier/proof, pure model-origin planning, proof-before-policy/approval/provider dispatch, effect-ceiling enforcement, a metadata-only tool-executor-guarded wrapper, bounded typed execution middleware, and invariant-lifetime Nested-origin sessions. Nested calls reuse the pinned registry and exact guarded raw dispatch while inheriting root identity, cancellation, deadline, output/effect ceilings and bounded depth/count/cycle state; compile-fail tests prevent lifetime escape, cloning and conversion to model sessions. Exact generated AgentContext/consumer-edge proof wiring, borrowed Command origin, explicit cost/concurrency scheduling, driver-tools and generated tool composition remain. |
