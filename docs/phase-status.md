...
| 3 — tool execution plane | In progress | Slices 3.1–3.4 implement the bounded Tool DTO/output/policy and opaque registration boundary, PermissionPolicy/Approval, permission-default, provider-free Prompt/Compaction/Attachment/Spill/Telemetry contracts, and the first guarded reference-monitor core: sealed provider snapshots, private deterministic registry, exact ToolCall issuer/verifier/proof, pure model-origin planning, proof-before-policy/approval/provider dispatch, effect-ceiling enforcement, and a metadata-only tool-executor-guarded wrapper. Exact generated AgentContext/consumer-edge proof wiring, typed middleware, borrowed Command/Nested execution, bounded scheduling, driver-tools and generated tool composition remain. |
