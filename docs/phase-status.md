...
| 3 — tool execution plane | In progress | Slices 3.1–3.5 implement the bounded Tool DTO/output/policy and opaque registration boundary, PermissionPolicy/Approval, permission-default, provider-free Prompt/Compaction/Attachment/Spill/Telemetry contracts, and the guarded reference-monitor core: sealed provider snapshots, private deterministic registry, exact ToolCall issuer/verifier/proof, pure model-origin planning, proof-before-policy/approval/provider dispatch, effect-ceiling enforcement, a metadata-only tool-executor-guarded wrapper, and bounded typed execution middleware with deterministic entry/exit order, stage-exact fail-closed policy, best-effort result observation, immutable context, read-only cancellation state and fixed deadline lineage. Exact generated AgentContext/consumer-edge proof wiring, borrowed Command/Nested execution, bounded scheduling, driver-tools and generated tool composition remain. |
