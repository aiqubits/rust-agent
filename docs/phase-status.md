...
| 4 — local execution providers | In progress | Slices 4.1–4.4 add the effect-free bounded filesystem API, exact projected resource bootstrap, production Linux descriptor-relative `fs-read-local`/`fs-local`, provider-neutral `tool-fs`, and the closed process/confinement/shell/terminal API. Local filesystem roots remain descriptor anchored across pathname replacement; symlink, hard-link, TOCTOU, cursor, cancellation/deadline, write and bounded `.gitignore`-aware search regressions are mapped. Subprocess now has no raw `ProcessSpec` spawn path: an Agent-scoped issuer/verifier pair seals a ceiling-projected, digest-bound, non-cloneable `ConfinedProcessSpec`, and returned handles require post-setup enforcement evidence. Generated namespace/factory/confinement wiring and the Linux subprocess/sandbox/shell/terminal/tool-shell providers remain. |
