...
| 4 — local execution providers | In progress | Slice 4.1 adds the effect-free, bounded `rust-agent-fs` API. Slice 4.2 adds the exact projected `cap:resource-namespace-bootstrap` protocol and production Linux `resource-namespace-bootstrap-local`: schema-owned commitments, private prepared state, deferred descriptor-relative `openat2`, symlink denial and root-replacement regressions are all mapped. Concrete filesystem/tool/process providers remain. |
