...
| 4 — local execution providers | In progress | Slices 4.1–4.6 add the effect-free bounded filesystem API, exact projected resource bootstrap, production Linux descriptor-relative `fs-read-local`/`fs-local`, provider-neutral `tool-fs`, the closed process/confinement/shell/terminal API, the Linux sandbox policy planner, and the verifier-only Linux subprocess provider. Local filesystem roots remain descriptor anchored across pathname replacement; symlink, hard-link, TOCTOU, cursor, cancellation/deadline, write and bounded `.gitignore`-aware search regressions are mapped. `sandbox-linux` alone receives the generated issuer and emits no side effect; `subprocess-local` receives only the paired verifier, its factory is I/O-free, and its typed lifecycle owner pins backend/launcher identity during initialization, rejects work after shutdown and applies namespace, Landlock, seccomp, no-new-privileges, process-group and resource enforcement before its policy-bound setup acknowledgement. The real subprocess gate is assigned to the privileged Ubuntu runner because this local host disables the required unprivileged namespace setup. Generated lifecycle/namespace/factory/confinement wiring and the Linux shell/terminal/tool-shell providers remain. |
