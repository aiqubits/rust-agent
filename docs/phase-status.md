...
| 3 — tool execution plane | Complete | Slices 3.1–3.13 implement the bounded Tool DTO/output/policy and opaque registration boundary, PermissionPolicy/Approval, permission-default, provider-free Prompt/Compaction/Attachment/Spill/Telemetry contracts, provider-neutral bounded model Tool-call events, the API-only bounded `driver-tools` loop, and the guarded reference-monitor core with sealed provider snapshots, a private deterministic registry, exact journal issuer/verifier/proof, proof-before-policy/approval/provider dispatch, effect ceilings, typed middleware, invariant-lifetime Nested/Command authority, checked cost accounting, and bounded model-order scheduling. Generated Agent scope assembly records the exact selected-driver → ToolExecutor edge, retains the issuer only in AgentContext, projects the required runtime primitives, and constructs permission-default → tool-executor-guarded → driver-tools in dependency order. An isolated locked generated composition builds and executes the request path, while Cargo tree, metadata and lockfile assertions prove the Tool packages are present and absent from the minimal composition. Compile-fail suites close wrapper internals, permit/policy/proof forgery and raw model execution; sessionless proof execution, pre-callback rejection and the targeted cancellation matrix are runnable evidence. Every item in the Phase 3 prose acceptance block is mapped in `docs/invariant-tests.md`, and the API closure, pinned target matrix, generated graph and acceptance-mapping CI gates pass. Later phases will connect non-empty CommandDispatcher issuance to its journal gate and supply concrete Prompt/Compaction/Attachment providers without reopening the completed Phase 3 capability contracts. |
| 4 — local execution providers | In progress | Slices 4.1–4.9 add the effect-free bounded filesystem API, exact projected resource bootstrap, production Linux descriptor-relative `fs-read-local`/`fs-local`, provider-neutral `tool-fs`, the closed process/confinement/shell/terminal API, the Linux sandbox policy planner, the verifier-only Linux subprocess provider, pure `shell-local`/`terminal-local` adapters, and the API-only bounded `tool-shell`. Local filesystem roots remain descriptor anchored across pathname replacement; symlink, hard-link, TOCTOU, cursor, cancellation/deadline, write and bounded `.gitignore`-aware search regressions are mapped. `sandbox-linux` alone receives the generated issuer and emits no side effect; `subprocess-local` receives only the paired verifier, its factory is I/O-free, and its typed lifecycle owner pins backend/launcher identity during initialization, rejects work after shutdown and applies namespace, Landlock, seccomp, no-new-privileges, process-group and resource enforcement before its policy-bound setup acknowledgement. Its exact terminal mode additionally owns a real PTY, controlling-terminal setup, bounded interactive output and live read/write/resize, while `terminal-local` preserves the only permitted Terminal → Sandbox → Subprocess chain and bounded teardown. `tool-shell` requires only the exact-effect `ShellBinding`, requests a closed deny-network sandbox policy, propagates cancellation, and emits bounded normalized output without importing any concrete shell, sandbox or subprocess provider. The real subprocess/PTY gate is assigned to the privileged Ubuntu runner because this local host disables the required unprivileged namespace setup. Generated lifecycle/namespace/factory/confinement wiring remains. |
...
- schema-4 policy optionally declares one closed Host-linker bundle. Its linker and sorted unique
  helper ids resolve to separately digest/version-bound executables and requirements must select
  all or none. It also selects the exact `wasm32-unknown-unknown` Target linker independently from
  Component requirements. The schema-3 path-free enforcement identity binds the exact logical
  Target-linker mount/config, Host build-triple linker configuration and `COMPILER_PATH`; accepted
  ADRs 0009–0012 additionally require Host-only Cargo linker/rustflag configuration, Target-only
  explicit sysroot flags, schema-5 planner configuration, backend semantic version 6, the closed
  inherited `CargoDriverEnvironmentV1`, execution of only the selected Host helpers, and an exact
  count of Target-linker executions for link-producing Target rustc units. The implementation and
  observation regressions pass locally; old schemas, missing/duplicate/overlapping linkers,
  alternate or cross-kind flags and partial Host selection fail closed before Cargo;
...
- the corresponding closed, bounded schema-4 probe request fixes Cargo to `-V`, rustc to `-vV`,
  selected build executables to `--version`, and the Target linker to its exact LLD probe, with
  distinct Host-linker, Host-linker-helper and Target-linker roles. Its schema-3 observation must
  contain the exact ordered role/id/digest/argv set, successful exits and exact
...
- verified raw unit graphs normalize into canonical `HostCargoUnitGraph` only when the Host input
  ...
  covers every raw edge exactly once. Each normalized edge retains Cargo's exact extern-crate name,
  including dependency aliases, in canonical graph ordering, digest and planned/observed equality;
  invalid or renamed observation drift fails closed. Cross-compile build scripts, proc macros and their transitive
...
dependency-isolation, generated-package and end-to-end gate passes. Phase 3 is independently
complete under its own mapped gates above. Neither completion claims Phase 5 durable Session
behavior, production browser execution, or production support beyond the targets declared by the
selected Components.
