...
| 1A — generated graph proof | In progress | The development-only generator/resolver/build path, path-free compose rustc executable/version/full-sysroot provenance, canonical target-fact and custom-spec snapshots with rustc/Cargo before/after drift checks, schema-owned Cargo package-metadata discovery, bounded/shared target-support analysis, bounded metadata/profile inputs, an identity-bound normalized catalog/trust-policy/evidence-byte/root-requirement generator-input commitment with resolver/attribution/generated-source/source-closure rederivation, selected-evidence snapshot verification, conservative aggregate App handoff, committed-built-in-fact Cargo target-dependency rewriting with transitive active path-package snapshots, checked canonical resolution/manifests, exact Cargo.lock source projection, Cargo-config ancestor rejection, source snapshots, real graph presence/absence, integration verification, topology fixtures, target matrix and WASM packaging are implemented. Remaining gates include generated shared-host Config-field type/identity sealing plus a real two-App same-identity/no-reopen compile/runtime fixture; direct-serde early resource closure for remaining manifest/diagnostic/Cargo-source collections and composition-wide source bounds; a real pinned-toolchain custom-target compose/lock/build test; and completion of every exact acceptance mapping/gate. Phase 1A artifacts remain `deployable=false`; local before/after filesystem checks are not a same-UID immutable-view proof. |
