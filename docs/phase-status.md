...
| 3 — tool execution plane | In progress | Slices 3.1–3.8 implement the bounded Tool DTO/output/policy and opaque registration boundary, PermissionPolicy/Approval, permission-default, provider-free Prompt/Compaction/Attachment/Spill/Telemetry contracts, and the guarded reference-monitor core: sealed provider snapshots, private deterministic registry, exact ToolCall issuer/verifier/proof, pure model-origin planning, proof-before-policy/approval/provider dispatch, effect-ceiling enforcement, a metadata-only tool-executor-guarded wrapper, bounded typed execution middleware, invariant-lifetime Nested/Command-origin sessions, and model-origin rolling bounded scheduling. The scheduler admits only one exact cancellation/deadline/runtime lineage, treats unknown/default as global exclusive, permits explicitly parallel calls up to a checked hard cap, serializes equal derived resource keys, stops new dispatch after cancellation/deadline failure, records a guarded terminal outcome for every admitted call and returns outcomes in model order. Nested calls reuse the pinned registry and exact guarded raw dispatch while inheriting root identity, cancellation, deadline, output/effect ceilings and bounded depth/count/cycle state. Command grants are opaque, exact-permit borrowed and carry bounded call/output/effect authority; `prepare_command` additionally binds Agent/lifecycle, executor digest and the exact clock/sleep projection without exposing runtime primitives. Compile-fail tests prevent authority forgery, cloning, lifetime escape and conversion to model sessions. Later phases will connect non-empty CommandDispatcher issuance to its journal gate. Phase 3 still requires exact generated AgentContext/consumer-edge proof wiring, explicit cost accounting, driver-tools and generated tool composition. |
