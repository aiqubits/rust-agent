...
| 4 — local execution providers | In progress | Slices 4.1–4.3 add the effect-free bounded filesystem API, exact projected resource bootstrap, production Linux descriptor-relative `fs-read-local`/`fs-local`, and provider-neutral `tool-fs`. Local filesystem roots remain descriptor anchored across pathname replacement; symlink, hard-link, TOCTOU, cursor, cancellation/deadline, write and bounded `.gitignore`-aware search regressions are mapped. Generated namespace/factory wiring and the subprocess/sandbox/shell/terminal/tool-shell block remain. |
