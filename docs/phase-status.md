...
| 4 — local execution providers | In progress | Slice 4.1 adds the effect-free, bounded `rust-agent-fs` API with canonical logical paths, call/range/page ceilings, provider-bound stable cursors, output revalidation and compile-fail privacy coverage. Local namespace/bootstrap and concrete filesystem/process providers remain. |
