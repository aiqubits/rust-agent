...
| 4 — local execution providers | In progress | Slices 4.1–4.5 add the effect-free bounded filesystem API, exact projected resource bootstrap, production Linux descriptor-relative `fs-read-local`/`fs-local`, provider-neutral `tool-fs`, the closed process/confinement/shell/terminal API, and the Linux sandbox policy planner. Local filesystem roots remain descriptor anchored across pathname replacement; symlink, hard-link, TOCTOU, cursor, cancellation/deadline, write and bounded `.gitignore`-aware search regressions are mapped. `sandbox-linux` alone receives the generated issuer, monotonically projects every request through the immutable Agent ceiling, binds the exact Linux primitive plan and emits no enforcement report or process side effect. Generated namespace/factory/confinement wiring and the Linux subprocess/shell/terminal/tool-shell providers remain. |
