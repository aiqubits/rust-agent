...
| 4 — local execution providers | In progress | Slices 4.1–4.8 add the effect-free bounded filesystem API, exact projected resource bootstrap, production Linux descriptor-relative `fs-read-local`/`fs-local`, provider-neutral `tool-fs`, the closed process/confinement/shell/terminal API, the Linux sandbox policy planner, the verifier-only Linux subprocess provider, and pure `shell-local`/`terminal-local` adapters. Local filesystem roots remain descriptor anchored across pathname replacement; symlink, hard-link, TOCTOU, cursor, cancellation/deadline, write and bounded `.gitignore`-aware search regressions are mapped. `sandbox-linux` alone receives the generated issuer and emits no side effect; `subprocess-local` receives only the paired verifier, its factory is I/O-free, and its typed lifecycle owner pins backend/launcher identity during initialization, rejects work after shutdown and applies namespace, Landlock, seccomp, no-new-privileges, process-group and resource enforcement before its policy-bound setup acknowledgement. Its exact terminal mode additionally owns a real PTY, controlling-terminal setup, bounded interactive output and live read/write/resize, while `terminal-local` preserves the only permitted Terminal → Sandbox → Subprocess chain and bounded teardown. The real subprocess/PTY gate is assigned to the privileged Ubuntu runner because this local host disables the required unprivileged namespace setup. Generated lifecycle/namespace/factory/confinement wiring and `tool-shell` remain. |
