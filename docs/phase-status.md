...
- schema-3 policy optionally declares one closed Host-linker bundle. Its linker and sorted unique
  ...
  backend semantic version 4 and execution of only the selected helpers. Their implementation and
  observation regressions are the current open work; old schemas, missing/duplicate helpers and
  partial selection fail closed before Cargo;
...
proved global encoded rustflags affect Target rather than Host units. Their implementation, local
baseline and a fresh successful Ubuntu 24.04 Phase 1B run are the remaining evidence gates. The
local host's Landlock ABI 1 still cannot supply the required `Refer` enforcement.
